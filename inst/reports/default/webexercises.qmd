...
If present, the feedback vector must be the same length as the options vector and the order must match up with the order of the options vector. However, you
can pass an empty string "" if you don't want any feedback to show for certain options.

- What is 20 - 2 x 4? `r mcq(opts=c("72", answer = "12"), feedback=c("Not quite - remember the order of operations. What part of the sum do you need to do first?", "Great job! You followed the operations by doing the multiplication before the subtraction."))`
